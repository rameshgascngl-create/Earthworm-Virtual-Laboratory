...
public final class NativeHomeActivity extends Activity {
    ...
    @Override public void onCreate(Bundle savedInstanceState){
        ...
        TextView sub=text(
            "Native Android generation · Metaphire posthuma",
            16,Color.rgb(185,211,203),false);
        sub.setPadding(0,dp(5),0,dp(18));
        body.addView(sub);

        ProgressStore store=new ProgressStore(this);
        ContentRepository content=new ContentRepository(this);

        addCard(
            "Continue native laboratory",
            "Resume the last native system. No WebView is involved.",
            store.getLastSystem());

        for(String systemId:content.systemIds()){
            addCard(
                content.systemName(systemId,false),
                systemSummary(systemId),
                systemId);
        }

        Button guided=button("Guided practical · 56 actions");
        guided.setOnClickListener(v->startActivity(new Intent(this,GuidedActivity.class)));
        body.addView(guided,new LinearLayout.LayoutParams(-1,dp(52)));

        Button microscopy=button("Microscopy · 9 lessons");
        microscopy.setOnClickListener(v->startActivity(new Intent(this,MicroscopyActivity.class)));
        LinearLayout.LayoutParams mp=new LinearLayout.LayoutParams(-1,dp(52));
        mp.setMargins(0,dp(10),0,0);
        body.addView(microscopy,mp);

        Button assessment=button("Native assessment · 72 questions");
        assessment.setOnClickListener(v->startActivity(new Intent(this,AssessmentActivity.class)));
        ...
        TextView footer=text(
            ...
            " · native Views/Canvas · offline · no account · no ads\n"+
            "Visited structures: "+store.visitedCount()+
            " · assessment score: "+store.score(),

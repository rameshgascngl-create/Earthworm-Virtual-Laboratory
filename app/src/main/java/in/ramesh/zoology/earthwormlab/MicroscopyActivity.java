...
public final class MicroscopyActivity extends Activity {
    ...
    private void render(){
        ...
        for(ContentRepository.Microscopy m:repo.microscopy()){TextView card=text((tamil&&!m.ta.trim().isEmpty()?m.ta:m.en)+"\n"+(tamil&&!m.textTa.trim().isEmpty()?m.textTa:m.textEn),15,Color.rgb(226,242,237),false);card.setLineSpacing(0,1.16f);card.setPadding(dp(14),dp(16),dp(14),dp(18));body.addView(card);}

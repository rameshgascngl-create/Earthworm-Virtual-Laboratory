...
if(pkg.version==='1.3.3-beta'){for(const [key,expected] of Object.entries(baseline.dataHashes))check(sha(JSON.stringify(data[key]))===expected,'Reviewed educational data preserved: '+key);}else{for(const key of Object.keys(baseline.dataHashes))check(!!data[key],'Educational dataset present: '+key);}
const svg=html.match(/<svg id="anatomySvg"[\s\S]*?<\/svg>/)[0],styles=[...html.matchAll(/<style\b[^>]*>([\s\S]*?)<\/style>/g)].map(m=>m[1]);
if(pkg.version==='1.3.3-beta'){check(sha(svg)===baseline.renderingContext.svgSHA256,'SVG markup matches the reviewed rendering baseline');}else{check(!!svg&&svg.includes('anatomySvg'),'Anatomy SVG present');}
if(pkg.version==='1.3.3-beta'){check(JSON.stringify(styles.map(sha))===JSON.stringify(baseline.renderingContext.styleSHA256),'Style blocks match the reviewed rendering baseline');}else{check(styles.length>=1,'Style blocks present');}

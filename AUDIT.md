...
| API 36 lint rejected the legacy onBackPressed override. | Retained that method only as the API 24–32 fallback and added a narrow GestureBackNavigation annotation; API 33+ still uses the registered platform predictive-Back callback. | GitHub run 2 passed the complete browser gate, then stopped at this single lint error before packaging. |
